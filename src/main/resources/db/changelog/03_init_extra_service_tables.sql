...
(
    extra_service_id                int GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    extra_service_name varchar NOT NULL,
    CONSTRAINT extra_service_pk PRIMARY KEY (extra_service_id)
);

COMMENT ON TABLE extra_service IS 'Таблица дополнительных услуг';
COMMENT ON COLUMN extra_service.extra_service_id IS 'Идентификатор дополнительной услуги';
COMMENT ON COLUMN extra_service.extra_service_name IS 'Наименование дополнительной услуги';

CREATE TABLE extra_service_price
(
    extra_service_price_id               int GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    price            numeric NOT NULL,
    is_actual        boolean DEFAULT true,
    extra_service_id int     NOT NULL REFERENCES extra_service(extra_service_id) ON DELETE CASCADE,
    CONSTRAINT extra_service_price_pk PRIMARY KEY (extra_service_price_id)
);

COMMENT ON TABLE extra_service_price IS 'Таблица стоимости дополнительных услуг';
COMMENT ON COLUMN extra_service_price.extra_service_price_id  IS 'Идентификатор стоимости услуги';
